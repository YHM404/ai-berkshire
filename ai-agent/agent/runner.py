...
5. read_file / write_file 仅用于读取已有资料或保存中间数据
6. **[关键] 最终研究报告必须通过 submit_research_report(report_markdown=...) 提交**。不要用 write_file 保存，也不要在对话中直接输出完整报告。
...
- 所有工具调用完成后，必须输出一份完整的、可读的研究报告，不要只说"分析完成"
"""
...
def build_agent(
    ...
    if extra_system_instructions.strip():
